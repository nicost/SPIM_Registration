...
public class InteractiveIntegral implements PlugIn
{
	...
	int channel = 0;
	Rectangle rectangle;
	Img<FloatType> source, sliceImage;
	Img<LongType> integralImage;
	...
	protected double minIntensity = Double.NaN, maxIntensity = Double.NaN;
	...
	public void setMinIntensity( final double min ) { this.minIntensity = min; }
	public void setMaxIntensity( final double max ) { this.maxIntensity = max; }
	...
	public void run( String arg )
	{
		...
		sliceImage = source.factory().create( new long[]{ source.dimension( 0 ), source.dimension( 1 ) }, source.firstElement() );
		
		// compute min/max
		if ( Double.isNaN( minIntensity ) || Double.isNaN( maxIntensity ) )
		{
			final float[] minmax = FusionHelper.minMax( source );
		...
		{
			this.min = (float)minIntensity;
			this.max = (float)maxIntensity;
		}

		IOFunctions.println( "Min intensity = " + this.min + ", max intensity = " + this.max );
	...
	public Img<LongType> computeIntegralImage( final Img<FloatType> img )
	{
	...
	final public static void computeDifferencOfMeanSlice( final Img< LongType> integralImg, final Img< FloatType > sliceImg, final int z, final int sx1, final int sy1, final int sz1, final int sx2, final int sy2, final int sz2, final float min, final float max  )
	{
		...
		final int w = (int)sliceImg.dimension( 0 ) - ( Math.max( sx1, sx2 ) / 2 ) * 2;
		final int h = (int)sliceImg.dimension( 1 ) - ( Math.max( sy1, sy2 ) / 2 ) * 2;
		final int d = ((int)integralImg.dimension( 2 ) - 1) - ( Math.max( sz1, sz2 ) / 2 ) * 2;
        ...
        for (int ithread = 0; ithread < threads.length; ++ithread)
            threads[ithread] = new Thread(new Runnable()
            {
                public void run()
                {
            		...
            		final Cursor< FloatType > cursor = sliceImg.localizingCursor();
            		final RandomAccess< LongType > randomAccess = integralImg.randomAccess();

            		cursor.jumpFwd( myChunk.getStartPosition() );
		...
		if ( peaks != null )
		{
			...
			{	
				if ( ( peak.isMax && lookForMaxima ) || ( peak.isMin && lookForMinima ) )
				{
					...
					{
						final OvalRoi or = new OvalRoi( Math.round( x - avgSize ), Math.round( y - avgSize ), radius1+radius2+1, radius1+radius2+1 );
	...
	public static ArrayList<SimplePeak> findPeaks( final Img<FloatType> laPlace, final float minValue )
	{
	    final AtomicInteger ai = new AtomicInteger( 0 );
...
MainLoop:           while ( cursor.hasNext() )
	...
	public static Img<FloatType> convertToFloat( final ImagePlus imp, int channel, int timepoint )
	{
		...
		if ( imp.getNSlices() > 1 )
			img = ArrayImgs.floats( new long[]{ imp.getWidth(), imp.getHeight(), imp.getNSlices() } );
		else
			img = ArrayImgs.floats( new long[]{ imp.getWidth(), imp.getHeight() } );
